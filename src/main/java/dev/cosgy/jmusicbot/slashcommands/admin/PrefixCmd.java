...
public class PrefixCmd extends AdminCommand {
    ...
    protected void execute(SlashCommandEvent event) {
        ...
        if (prefix.toLowerCase().matches("(none)")) {
            ...
            s.setPrefix(prefix);
            event.reply(event.getClient().getSuccess() + "*" + event.getGuild().getName() + "* The prefix in `" + prefix + "`has been set to.").queue();
        }
    }
    ...
    protected void execute(CommandEvent event) {
        if (event.getArgs().isEmpty()) {
            event.replyError("Include the prefix or NONE.");
            return;
        }
        ...
        if (event.getArgs().toLowerCase().matches("(none|なし)")) {
            ...
            event.replySuccess("*" + event.getGuild().getName() + "* The prefix in `" + event.getArgs() + "`has been set.");
        }
    }
}
