...
public class SetstatusCmd extends OwnerCommand {
    public SetstatusCmd(Bot bot) {
        this.name = "setstatus";
        this.help = "Set the status the bot will display";
    ...
    protected void execute(SlashCommandEvent event) {
        try {
            ...
            if (status == OnlineStatus.UNKNOWN) {
                ...
                event.reply(event.getClient().getSuccess() + "Status`" + status.getKey().toUpperCase() + "`has been set to.").queue();
            }
        } catch (Exception e) {
    ...
    protected void execute(CommandEvent event) {
        try {
            ...
            if (status == OnlineStatus.UNKNOWN) {
                ...
                event.getJDA().getPresence().setStatus(status);
                event.replySuccess("Status`" + status.getKey().toUpperCase() + "`has been set to.");
            }
        } catch (Exception e) {
