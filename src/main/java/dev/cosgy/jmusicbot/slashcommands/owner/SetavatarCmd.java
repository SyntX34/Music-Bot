...
public class SetavatarCmd extends OwnerCommand {
    public SetavatarCmd(Bot bot) {
        this.name = "setavatar";
        this.help = "Set the bot's avatar";
        this.arguments = "<url>";
    ...
    protected void execute(SlashCommandEvent event) {
        ...
        if (s == null) {
            event.reply(event.getClient().getError() + " Invalid or Missing URLs").queue();
        } else {
            try {
                event.getJDA().getSelfUser().getManager().setAvatar(Icon.from(s)).queue(
                        v -> event.reply(event.getClient().getSuccess() + "Changed your avatar.").queue(),
    ...
    protected void execute(CommandEvent event) {
        ...
        InputStream s = OtherUtil.imageFromUrl(url);
        if (s == null) {
            event.reply(event.getClient().getError() + " Invalid or Missing URLs");
        } else {
            try {
                event.getSelfUser().getManager().setAvatar(Icon.from(s)).queue(
                        v -> event.reply(event.getClient().getSuccess() + "Changed your avatar."),
