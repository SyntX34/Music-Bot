...
public class UserInfo extends SlashCommand {
    ...
    protected void execute(SlashCommandEvent event) {
        ...
        String STATUS = memb.getOnlineStatus().getKey().replace("offline", ":x: Offline").replace("dnd", ":red_circle: Do not disturb").replace("idle", "Away").replace("online", ":white_check_mark: Online");
        String ROLES;
        String GAME;
        ...
        log.debug("\nUsername:" + memb.getEffectiveName() + "\n" +
            ...
            "Tags:" + memb.getUser().getDiscriminator() + "\n" +
        ...
        eb.setAuthor(memb.getUser().getName() + TAG + " User info", null, null)
    ...
    public void execute(CommandEvent event) {
        ...
        if (ROLES.length() > 0)
            ...
            ROLES = "There are no roles on this server";
