...
public class SearchCmd extends MusicCommand {
    ...
    public SearchCmd(Bot bot) {
        ...
        this.help = "Searches for videos on YouTube using the specified query.";
    ...
    public void doCommand(CommandEvent event) {
        if (event.getArgs().isEmpty()) {
            event.replyError("Please specify a query.");
            return;
        }
        event.reply(searchingEmoji + "`[" + event.getArgs() + "]` Searching... ",
                m -> bot.getPlayerManager().loadItemOrdered(event.getGuild(), searchPrefix + event.getArgs(), new ResultHandler(m, event)));
    }

    @Override
    public void doCommand(SlashCommandEvent event) {
        event.reply(searchingEmoji + "`[" + event.getOption("input").getAsString() + "]` Searching... ").queue(
    ...
    private class SlashResultHandler implements AudioLoadResultHandler {
        ...
        public void trackLoaded(AudioTrack track) {
            ...
            m.editOriginal(FormatUtil.filter(event.getClient().getSuccess() + "**" + track.getInfo().title
                    ...
                    : "has been added as number " + pos + " in the queue."))).queue();
        }
            ...
            builder.setColor(event.getSelfMember().getColor())
                    ...
                    .setSelection((msg, i) -> {
                        ...
                        event.replySuccess("**" + FormatUtil.filter(track.getInfo().title)
                                + "** (`" + FormatUtil.formatTime(track.getDuration()) + "`) " + (pos == 0 ? "will start playing."
                                : "has been added as number " + pos + " in the queue."));
                    })
                    .setCancel((msg) -> {
                    })
