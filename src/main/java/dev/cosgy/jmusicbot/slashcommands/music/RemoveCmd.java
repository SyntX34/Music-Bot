    ...
    public void doCommand(CommandEvent event) {
        ...
        if (event.getArgs().toLowerCase().matches("(all|all)")) {
            ...
            if (count == 0)
                ...
                event.replySuccess(count + " songs were removed.");
            return;
        }
        int pos;
        try {
        ...
        if (qt.getIdentifier() == event.getAuthor().getIdLong()) {
            ...
            event.replySuccess("**" + (qt.getTrack().getInfo().uri.contains("https://stream.gensokyoradio.net/") ? "Gensokyo Radio" : qt.getTrack().getInfo().title) + "** was removed from the queue.");
        } else if (isDJ) {
            handler.getQueue().remove(pos - 1);
            User u;
            try {
            ...
            event.replyError("**" + (qt.getTrack().getInfo().uri.contains("https://stream.gensokyoradio.net/") ? "Gensokyo Radio" : qt.getTrack().getInfo().title) + "** could not be removed. Reason: Do you have DJ permissions? You cannot remove requests that are not yours.");
        }
    }
    ...
    public void doCommand(SlashCommandEvent event) {
        ...
        if (event.getOption("input").getAsString().toLowerCase().matches("(all|all)")) {
            ...
            if (count == 0)
                ...
                event.reply(event.getClient().getSuccess() + count + " songs were removed.").queue();
            return;
        }
        int pos;
        try {
        ...
        if (qt.getIdentifier() == event.getUser().getIdLong()) {
            handler.getQueue().remove(pos - 1);
            event.reply(event.getClient().getSuccess() + "**" + qt.getTrack().getInfo().title + "** was removed from the queue.").queue();
        } else if (isDJ) {
            handler.getQueue().remove(pos - 1);
            User u;
            try {
            ...
            event.reply(event.getClient().getError() + "**" + qt.getTrack().getInfo().title + "** could not be removed. Reason: Do you have DJ permissions? You cannot remove requests that are not yours.").queue();
        }
    }
}