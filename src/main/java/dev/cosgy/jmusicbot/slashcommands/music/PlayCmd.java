...
public class PlayCmd extends MusicCommand {
    ...
    public void doCommand(CommandEvent event) {
        ...
        if (event.getArgs().isEmpty() && event.getMessage().getAttachments().isEmpty()) {
            ...
            if (handler.getPlayer().getPlayingTrack() != null && handler.getPlayer().isPaused()) {
                if (DJCommand.checkDJPermission(event)) {
                    ...
                    event.replySuccess("**" + handler.getPlayer().getPlayingTrack().getInfo().title + "** playback has resumed.");
                ...
                event.getChannel().sendMessage(":calling: Loading cache file... (" + cache.getItems().size() + "songs)").queue(m -> {
                    cache.loadTracks(bot.getPlayerManager(), (at) -> {
                        ...
                        StringBuilder builder = new StringBuilder(cache.getTracks().isEmpty()
                                ...
                                : event.getClient().getSuccess() + "**" + cache.getTracks().size() + "** songs loaded from cache file.");
        ...
        event.reply(loadingEmoji + "`[" + args + "]`Loading...", m -> bot.getPlayerManager().loadItemOrdered(event.getGuild(), args, new ResultHandler(m, event, false)));
    }
        ...
        if (event.getOption("input") == null) {
            ...
            if (handler.getPlayer().getPlayingTrack() != null && handler.getPlayer().isPaused()) {
                if (DJCommand.checkDJPermission(event.getClient(), event)) {
                    ...
                    handler.getPlayer().setPaused(false);
                    event.reply(event.getClient().getSuccess() + "**" + handler.getPlayer().getPlayingTrack().getInfo().title + "** playback has resumed.").queue();
            ...
            if (bot.getCacheLoader().cacheExists(event.getGuild().getId())) {
                ...
                AtomicInteger count = new AtomicInteger();
                CacheLoader.CacheResult cache = bot.getCacheLoader().ConvertCache(data);
                event.reply(":calling: Reading cache file... (" + cache.getItems().size() + "曲)").queue(m -> {
                    cache.loadTracks(bot.getPlayerManager(), (at) -> {
                        // TODO:Use a cached user ID.
            ...
            if (handler.playFromDefault()) {
                ...
                Playlist playlist = bot.getPlaylistLoader().getPlaylist(event.getGuild().getId(), settings.getDefaultPlaylist());
                if (playlist == null) {
                    event.reply("Could not find `" + event.getOption("input").getAsString() + ".txt` in playlist folder.").queue();
                    return;
                }
    ...
    public class SlashResultHandler implements AudioLoadResultHandler {
        ...
        private void loadSingle(AudioTrack track, AudioPlaylist playlist) {
            if (bot.getConfig().isTooLong(track)) {
                m.editOriginal(FormatUtil.filter(event.getClient().getWarning() +
                        " **" + (track.getInfo().uri.matches(".*stream.gensokyoradio.net/.*") ? "Gensokyo Radio" : track.getInfo().title) + "**`(" + FormatUtil.formatTime(track.getDuration()) + ")` は設定された長さ`(" + FormatUtil.formatTime(bot.getConfig().getMaxSeconds() * 1000) + ")` を超えています。")).queue();
                return;
            }
            AudioHandler handler = (AudioHandler) event.getGuild().getAudioManager().getSendingHandler();
            ...
            // Output MSG ex:
            // Added <title><(length)>.
            // Added <title><(length)> to the <playback queue number>.
            String addMsg = FormatUtil.filter(event.getClient().getSuccess() + " **" + (track.getInfo().uri.matches(".*stream.gensokyoradio.net/.*") ? "Gensokyo Radio" : track.getInfo().title)
                + "** (`" + FormatUtil.formatTime(track.getDuration()) + "`) " + (pos == 0 ? " was added. " : " was added to the " + pos + " queue."));
            if (playlist == null || !event.getGuild().getSelfMember().hasPermission(event.getTextChannel(), Permission.MESSAGE_ADD_REACTION)) {
                ...
                new ButtonMenu.Builder()
                        .setText(addMsg + "\n" + event.getClient().getWarning() + "This playlist contains **" + playlist.getTracks().size() + "** other songs. Choose " + LOAD + " to load the tracks.")
                        ...
                        .setAction(re ->
                        {
                            if (re.getName().equals(LOAD))
                                m.editOriginal(addMsg + "\n" + event.getClient().getSuccess() + "**" + loadPlaylist(playlist, track) + "** Song added to queue!").queue();
                            else
                                m.editOriginal(addMsg).queue();
                        }).setFinalAction(m ->
                        {
            ...
            if (playlist.getTracks().size() == 1 || playlist.isSearchResult()) {
                ...
                if (count == 0) {
                    ...
                    m.editOriginal(FormatUtil.filter(event.getClient().getSuccess()
                            + (playlist.getName() == null ? "Playlist" : "Playlist **" + playlist.getName() + "**") + " Song(s) in `"
                            + playlist.getTracks().size() + "` have been added to the queue."
                            + (count < playlist.getTracks().size() ? "\n" + event.getClient().getWarning() + " Track longer than the maximum allowed (`"
                            + bot.getConfig().getMaxTime() + "`) was truncated." : ""))).queue();
                }
            }
        }
        ...
        public void loadFailed(FriendlyException throwable) {
            if (throwable.severity == Severity.COMMON) {
                ...
                m.editOriginal(event.getClient().getError() + "An error occurred while loading songs.").queue();
            }
        }
    }
    ...
    private class ResultHandler implements AudioLoadResultHandler {
        ...
        private void loadSingle(AudioTrack track, AudioPlaylist playlist) {
            if (bot.getConfig().isTooLong(track)) {
                m.editMessage(FormatUtil.filter(event.getClient().getWarning() +
                    " **" + track.getInfo().title + "**`(" + FormatUtil.formatTime(track.getDuration()) + ")` exceeds the configured length`(" + FormatUtil.formatTime(bot.getConfig().getMaxSeconds() * 1000) + ")`.")).queue();
                return;
            }
            AudioHandler handler = (AudioHandler) event.getGuild().getAudioManager().getSendingHandler();
            ...
            // Output MSG ex:
            // <title><(length)> has been added.
            // <title><(length)> has been added to the <playback queue number>th queue.
            String addMsg = FormatUtil.filter(event.getClient().getSuccess() + " **" + (track.getInfo().uri.contains("https://stream.gensokyoradio.net/") ? "Gensokyo Radio" : track.getInfo().title)
                    + "** (`" + FormatUtil.formatTime(track.getDuration()) + "`) " + (pos == 0 ? "Added." : "Added to the " + pos + "th queue for playback."));
        ...
        public void playlistLoaded(AudioPlaylist playlist) {
            if (playlist.getTracks().size() == 1 || playlist.isSearchResult()) {
                ...
                int count = loadPlaylist(playlist, null);
                if (count == 0) {
                    m.editMessage(FormatUtil.filter(event.getClient().getWarning() + " A message in this playlist" + (playlist.getName() == null ? "" : "(**" + playlist.getName()
                            + "**) ") + " is longer than the maximum length allowed. (`" + bot.getConfig().getMaxTime() + "`)")).queue();
                } else {
        ...
        public void loadFailed(FriendlyException throwable) {
            if (throwable.severity == Severity.COMMON) {
                ...
                if (m.getAuthor().getIdLong() == bot.getConfig().getOwnerId() || m.getMember().isOwner()) {
                    m.editMessage(event.getClient().getError() + "An error occurred while loading songs.\n" +
                            "**Error: " + throwable.getLocalizedMessage() + "**").queue();
                    StackTraceUtil.sendStackTrace(event.getTextChannel(), throwable);
                    return;
                }

                m.editMessage(event.getClient().getError() + "There was an error loading songs.").queue();
            }
        }
    }
        ...
        public void doCommand(SlashCommandEvent event) {
            ...
            if (event.getOption("input") == null) {
                ...
                if (bot.getCacheLoader().cacheExists(event.getGuild().getId())) {
                    ...
                    AtomicInteger count = new AtomicInteger();
                    CacheLoader.CacheResult cache = bot.getCacheLoader().ConvertCache(data);
                    event.reply(":calling: Loading cache files...(" + cache.getItems().size() + "曲)").queue(m -> {
                        cache.loadTracks(bot.getPlayerManager(), (at) -> {
                            // TODO:Use a cached user ID.
                            handler.addTrack(new QueuedTrack(at, event.getUser()));
                            count.getAndIncrement();
    ...
    public class PlaylistCmd extends MusicCommand {
        ...
        public void doCommand(CommandEvent event) {
            ...
            event.getChannel().sendMessage(":calling: playlist **" + event.getArgs() + "Loading **... (" + playlist.getItems().size() + " songs)").queue(m ->
            {
                AudioHandler handler = (AudioHandler) event.getGuild().getAudioManager().getSendingHandler();
                playlist.loadTracks(bot.getPlayerManager(), (at) -> handler.addTrack(new QueuedTrack(at, event.getAuthor())), () -> {
                    StringBuilder builder = new StringBuilder(playlist.getTracks().isEmpty()
    ...
    public class MylistCmd extends MusicCommand {
        ...
        public void doCommand(SlashCommandEvent event) {
            ...
            event.reply(":calling: Loading mylist**" + name + "**... (" + playlist.getItems().size() + " songs)").queue(m ->
            {
                AudioHandler handler = (AudioHandler) event.getGuild().getAudioManager().getSendingHandler();
                playlist.loadTracks(bot.getPlayerManager(), (at) -> handler.addTrack(new QueuedTrack(at, event.getUser())), () -> {
                    StringBuilder builder = new StringBuilder(playlist.getTracks().isEmpty()
    ...
    public class PublistCmd extends MusicCommand {
        public PublistCmd(Bot bot) {
            ...
            this.arguments = "<name>";
            this.help = "Play public listings";
            this.beListening = true;
            this.bePlaying = false;
        ...
        public void doCommand(SlashCommandEvent event) {
            ...
            event.reply(":calling: playlist **" + name + "Loading **... (" + playlist.getItems().size() + " songs)").queue(m ->
            {
                AudioHandler handler = (AudioHandler) event.getGuild().getAudioManager().getSendingHandler();
                playlist.loadTracks(bot.getPlayerManager(), (at) -> handler.addTrack(new QueuedTrack(at, event.getUser())), () -> {
                    StringBuilder builder = new StringBuilder(playlist.getTracks().isEmpty()
