...
public class BotConfig {
    ...
    public void load() {
        ...
        try {
            // Get the configuration path (default is config.txt)
            ...
            // The config file is loaded and default values ​​are added.
            ...
            dbots = owner == 598487840644399124L; //334091398263341056 Original Owner
            ...
            if (token == null || token.isEmpty() || token.matches("(BOT_TOKEN_HERE | Paste your bot token here)")) {
                token = prompt.prompt("Enter your BOT tokens."
                        + "\nHere's how to get the token:"
            ...
            if (owner <= 0) {
                try {
                    owner = Long.parseLong(prompt.prompt("The owner user ID is not set or is not a valid ID."
                            + "\nPlease enter the user ID of the bot owner."
                            + "\nHow to get your User ID:"
                            + "\nhttps://github.com/SyntX34/Music-Bot/wiki/Finding-Your-User-ID"
                            + "\nOwner's user ID: "));
            ...
            if (write) {
                ...
                if (original == null) {
                    ...
                    mod = original.substring(original.indexOf(START_TOKEN) + START_TOKEN.length(), original.indexOf(END_TOKEN))
                            ...
                            .replace("0 // OWNER ID", Long.toString(owner)).replace("Paste your owner ID here", Long.toString(owner))
                            .trim();
                }
            ...
            prompt.alert(Prompt.Level.ERROR, CONTEXT, ex + ": " + ex.getMessage() + "\n\nLocation of configuration files: " + path.toAbsolutePath());
        }
    }
